...
```{r, echo = sol}
second_version_of_mpg <- mpg %>% 
  filter(cyl == 6) %>% 
  mutate(class <- as.character(class))

ggplot(second_version_of_mpg, aes(cty, hwy, color = class)) +
  geom_point() +
...
```{r, echo = sol}
# Function that finds the max of a sequence and finding the min that 
# goes with that max evaluated at function f
#
# Args:
#  f: arbitrary function
#  x: first number of a sequence
#  y: last number of sequence
#  s: increment of 's'
#
# Returns x and y value of a coordinate


d <- function(f, x, y, s){
  
  my_sequence <- seq(x,y,by=s)
  new_sequence <- c()
  
  #for loop that creates a vector called 'evaluated_seq'
  #evaluates each element of 'my_sequence' with function 'f' 
  for(i in 1:length(my_sequence)){
    evaluated_seq <- f(my_sequence[i]) 
    #new_sequence outputs the results of function f
    new_sequence[i] <- evaluated_seq
  }
  
  my_max <- max(new_sequence)
  my_min <- my_sequence[new_sequence==my_max]
  my_min <- min(my_min)
  return(list(my_max,my_min))
}
...
```{r, echo = sol}
# Function that finds a list of means and median for given vector of numbers 
#
# Arg:
#  x: vector of numbers
#
# Returns two lists: list of rollling mean and list of rolling medians

ss <- function(x) {
  #returns values of vector x that are not missing
  x <- x[!is.na(x)]
  a <- x[1]
  
  for(i in 2:length(x)) {
    #find the sum of value at position i-1 of vector a and value at position i of vector x 
    a[i] <- (a[i-1] + x[i])
    #divide vector a by vector of consecutive numbers that ends at length(x)
    a <- a / (1:length(x))
    #set my_vector to vector 'x'
    my_vector <- x
    new_x <- 0
  } 
  ...
  for(i in 1:length(my_vector)) {
    ...
    if(length(x) %% 2 == 0) {
      new_x[i] <- (ordered_x[floor((length(ordered_x) + 1) / 2)] +
                ordered_x[ceiling((length(ordered_x) + 1) / 2)]) / 2
    } else { 
        new_x[i] <- ordered_x[(length(ordered_x) + 1) / 2]
      }
    }
return(list(a,new_x))
}
...
```{r, echo = sol}
# Find the means of Sepal Length, Sepal Width, Petal Length, Petal Width for 
# each of the species
iris_df = iris 
iris_df = iris_df %>% 
  group_by(Species) %>% 
