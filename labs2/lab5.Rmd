...
1. In this call of `str_pad`, __This pads the string with x's on the left side with a total width of 4__.
```{r}
str_pad("a", width = 4, side = "left", pad = "x")
```

2. In this call of `str_pad`, __This pads the string with blanks on the left side with a total width of 3__.
```{r}
str_pad("a", width = 3, side = "left")
```

3. In this call of `str_pad`, __This pads the string with blanks on the left side with a total width of 10 for each string__.
```{r}
str_pad(c("a", "abc", "abcdef"), 10)
```

4. In this call of `str_pad`, __This pads the string "a" with blanks on the left side with a total width of 5, 10, and 20__.
```{r}
str_pad("a", c(5, 10, 20))
```

5. In this call of `str_pad`, __This pads the string "a" with "-", "_", and blanks on the left side with a total width of 10__.
```{r}
...
```{r}
any(str_detect(countries,regex("island",ignore_case = TRUE)))
...
str_subset(countries, "^[aA eE iI oO uU].*[^aA eE iI oO uU]")
```
