...
library(xml2)
setwd("~/Documents/Stat_133/stat133/labs5")
```
...
```{r, eval = TRUE}
x = read_xml('<document name="lab15" />')
xml_attr(x, "name")
```
2. Select all document elements beneath the node `linkList` with the name `A`.
```{r, eval = TRUE}
y = read_xml('<document xmlns:xlink="http://www.w3.org/1999/xlink">
  <linkList name="A">
...
xml_find_all(y, "/document/linkList[@name='A']/*")
```
...
xml_find_all(z, "//job[@priority='critical' or @priority='high']")
```
...
xml_find_all(b, "//artist[cds]/name/text()")
```
## XPathing
...
bk = read_xml('<?xml version="1.0" encoding="UTF-8"?>
