...
#Envelope because you can open and close
...
```{r}
ggplot()+
  geom_line(aes(exrate_times, gbp_rates))+
...
```{r}
GetExchangeRates = function(x=exrate.xml, currency){
  xml_root(x) %>% 
    xml_find_all(str_c("//Cube[@currency=", "'", currency, "'", "]")) %>%
    xml_attr("rate") %>% 
    as.numeric()
}
...
```{r}
currencies = lapply(c("GBP", "USD", "CAD", "AUD"), GetExchangeRates, x=exrate.xml)
...
exrate.df = do.call(data.frame, currencies) %>% 
  mutate(exrate_times = exrate_root %>% 
  xml_find_all('//Cube[@time]') %>%
  xml_attr("time") %>% 
  ymd())
