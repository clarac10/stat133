...
```{r}
# Problem 1
files100 = paste("plot", str_pad(seq(1,100), 3, "left", "0"), ".png", sep ="")
files100
```
...
reverse_chars = function(x){
  x=str_c(rev(substring(x,1:nchar(x),1:nchar(x))),collapse="")
  x
}
```
...
dna_to_aa = function(x){
  dna3 = codons$dna_triplet
  aa = codons$amino_acid
  string=""
  dna = substring(x, seq(1,nchar(x),3), seq(3,nchar(x),3))

  if(str_detect(x, "[^CGAT]") == TRUE){
  return("Invalid DNA Sequence.")
  } 
  else{
  for(i in 1:length(dna)){
  
  string[i]=aa[which(str_detect(dna3,dna[i]))]
}
}
