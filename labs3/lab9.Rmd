...
```{r}
split_pioneers = str_split(pioneers, ":", 2)
split_pioneers
```
...
```{r}
x = as.data.frame(pioneers_df, stringsAsFactors = FALSE)
names(x) = c("name", "byear")
x
```
...
```{r}
vapply(1:10, function(x) with(split_sleep[[x]], extra[group ==2])-with(split_sleep[[x]], extra[group ==1]), FUN.VALUE = numeric(1))
...
```{r}
nonsense_filter = filter(iris, str_count(iris$Species, "[AaEeIiOoUu]")==4, Sepal.Length + Sepal.Width < 8)
nonsense_filter
```
...
```{r}
iris$Sepal.Width = ifelse(iris$Sepal.Width > 3, "wide", "narrow")
iris$Sepal.Length = ifelse(iris$Sepal.Length > 5.5, "long", "short")
```
