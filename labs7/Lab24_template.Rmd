...
simMH = function(n, k, switch){
  ...
  win = c()
  doors = c(1:n)
  prize = resample(doors, 1, replace = FALSE)
  pick = resample(doors, 1, replace = FALSE)
  open = resample(doors[which(doors != pick & doors != prize)], k, replace = FALSE)
  newpick = resample(doors[-c(open,pick)], 1, replace = FALSE)
  
  
  if(switch == FALSE){
    win = ifelse(prize == pick, TRUE, FALSE)
    
  } else if(switch == TRUE){
    
    win = ifelse(newpick == prize, TRUE, FALSE)
